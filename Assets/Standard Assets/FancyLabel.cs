...
public class FancyLabel
{
    ...
    // #x        - bold font
    // #i        - italic font
    public static void Create(Rect rect, string text, GUIStyle baseStyle, TextAlignment alignment)
    {
        //bool   leftSpace = false, rightSpace = false, topSpace = false, bottomSpace = false;
        Font   newFont = null;
        //GUIStyle fontStyle = new GUIStyle(testStyle);
        GUIStyle fontStyle = new GUIStyle(baseStyle);
        Color textColor = fontStyle.normal.textColor;
        FontStyle fontStyleStyle = fontStyle.fontStyle;
        ...
        for(int i=0; i<toks.Length; ++i)
        {
            ...
            while(index < toks[i].Length)
            {
                ...
                else if(toks[i][index] == '#')
                {
                    ...
                    {
                        //Normal Font
                        fontStyleStyle = FontStyle.Normal;
                    }
                    else if(toks[i][index+1] == 'x')
                    {
                        //Bold Font
                        if(fontStyle.fontStyle == FontStyle.Italic) {
                            fontStyleStyle = FontStyle.BoldAndItalic;
                        }
                        else {
                            fontStyleStyle = FontStyle.Bold;
                        }
                    }
                    else if(toks[i][index+1] == 'i')
                    {
                        //Italic Font
                        if(fontStyle.fontStyle == FontStyle.Bold) {
                            fontStyleStyle = FontStyle.BoldAndItalic;
                        }
                        else {
                            fontStyleStyle = FontStyle.Italic;
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if(clearBuffer || newline > 0)
                    {
                        ...
                        fontStyle.fontStyle = fontStyleStyle;
                        if(newFont != null)
                        {
